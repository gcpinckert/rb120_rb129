...
=begin
The problem is that on line 35 we are using the task= method without self so Ruby
thinks we are initializing a local variable. Therefore, when tasks.select is called
it looks for the value referenced by local variable tasks and not the instance
variable @tasks. We can fix this by changing the name of the variable we want
to store our high priority tasks in to be more specific. That way, tasks.select
invokes the @tasks getter method, and an array of tasks calls the select method
rather than a nil object.
=end