...
=begin
The problem here was the fact that the Mailing module (which contains the #send method)
was not included in the PostalService class, only the EmailService class. We can
see that both the PostalService and the EmailService classes are dealing with
mail type behaviors, so we should mix the Mailing module into both of them. The
Telephone class, on the other hand, does not deal with mail and so should not include
the Mailing module.

Further, within out Mailing module we define the #send method which is defined in the
Object class. To avoid overriding this method, we should define a method with a more
specific name, such as #send_mail as seen here.
=end
